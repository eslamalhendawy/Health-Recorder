...
function GeneticModal() {
  ...
  async function submitGenetic() {
    if (surgery === "") {
      toast.error("Enter Name Of Surgery");
    } else if (date === "") {
      toast.error("Enter Surgery Date");
    } else {
      await axios
      ...
      {modal && (
        <div className="modal">
          ...
          <div className="modal-content max-w-[500px] bg-white px-6 py-4 rounded-xl text-[#212529] relative">
            <h2 className="mb-4 font-semibold sm:text-lg">Enter New Surgery Data</h2>
            <div className="sm:flex gap-4 mb-4">
              <div className="mb-4 flex flex-col basis-2/4">
                <span className="">Surgery Type :</span>
                <input onChange={(e) => setSurgery(e.target.value)} className="focus:outline-none  border-b-[2px] border-[#0cb8b6]" type="text" />
              </div>
